# Code Review: My Addons for room2 and fixes (PR #415)

**Reviewer:** FallenFate team  
**Scope:** `Assets/Sandbox/Angel_Rodriguez/Resources/Scripts/` and related Room2 systems  
**Reference:** [Unity Programming Best Practices](https://docs.unity3d.com/Manual/programming-best-practices.html), [Execution Order](https://docs.unity3d.com/Manual/execution-order.html)
...
- `[SerializeField] private GameObject actionDescriptionPrefab;`  
  (assign the prefab in the Inspector; Unity tracks it by GUID.)
- `[SerializeField] private Item_Data itemData;`  
  (assign the ScriptableObject asset; no path or Resources.Load needed.)
...
| `PickUpObjects.cs` | `Resources.Load<Item_Data>(scriptableObjectPath)` + `[SerializeField] private string scriptableObjectPath` | `[SerializeField] private Item_Data itemData;` – assign the **Item_Data** asset directly; remove `scriptableObjectPath` |
...
- [ ] **All prompt UI:** Cache `GetComponentsInChildren<Text>()` (or equivalent) when creating the prompt; avoid repeated GetComponent.
- [ ] **Asset paths → SerializeField:** Replace every `Resources.Load(...)` and string path (e.g. `scriptableObjectPath`) with `[SerializeField]` or public reference; assign assets in the Inspector so Unity tracks them by GUID (see exact paths table in §7).
...
Thanks for the Room2 addons; addressing these points will make the codebase more maintainable and performant.
