...
public class LocationSoTrigger : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    {
        if(other.CompareTag("Hitboxs") && !pickUpManager.discoveredLocations.Contains(location) && location != null)
        {
            pickUpManager.recordLocationDiscovery(location);
        }
        else if(other.CompareTag("Hitboxs") && pickUpManager.items.Contains(itemNeeded) && returnLocation != null)
        {
