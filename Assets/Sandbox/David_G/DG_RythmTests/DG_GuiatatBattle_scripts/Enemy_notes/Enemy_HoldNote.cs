...
namespace Dypsloom.RhythmTimeline.Core.Notes
{
    ...
    public class Enemy_HoldNote : Note
    {
        ...
        public override void Initialize(RhythmClipData rhythmClipData)
        {
            ...
            m_Holding = false;
            m_AutoPlayTapSent = false;
            m_AutoPlayReleaseSent = false;
        ...
        protected override void HybridUpdate(double timeFromStart, double timeFromEnd)
        {
            ...
            if (Application.isPlaying && m_AutoPlay && !m_IsTriggered) {
                var tapThreshold = m_AutoPlayPerfectTiming ? m_RhythmClipData.RhythmDirector.HalfCrochet : 0f;
                if (!m_AutoPlayTapSent && timeFromStart >= tapThreshold) {
                    m_AutoPlayTapSent = true;
                    OnTriggerInput(new InputEventData(RhythmClipData.TrackID, 0));
                }

                if (m_AutoPlayTapSent && !m_AutoPlayReleaseSent && m_Holding && deltaTEnd >= 0f) {
                    m_AutoPlayReleaseSent = true;
